...
//        lastCommandTimestamp = loadTimestamp;
    ...
    private void calculateStatusPlaylistOrAlbum() {
        switch (repeat) {
            case ("no repeat"):
                if (playMode.equals("album")) {
                    while (playTime > currentAlbum.getSongs().get(songIndexAlbum).getDuration()) {
                        // If is the playlist's last song, the player is reset
                        if (songIndexAlbum == currentAlbum.getSongs().size() - 1) {
                            originalAlbum.decreaseListeners();
                            originalAlbum.getSongs().get(songIndexAlbum).decreaseListeners();
                            playTime = 0;
                            paused = true;
                            ...
                            originalAlbum.getSongs().get(songIndexAlbum).decreaseListeners();
                            songIndexAlbum += 1;
                            originalAlbum.getSongs().get(songIndexAlbum).increaseListeners();
                        }
                    }
                } else {
                    while (playTime > currentPlaylist.getSongs().get(songIndex).getDuration()) {
                        ...
                        if (songIndex == currentPlaylist.getSongs().size() - 1) {
                            ...
                            originalPlaylist.getSongs().get(songIndex).decreaseListeners();
                            songIndex += 1;
                            originalPlaylist.getSongs().get(songIndex).increaseListeners();
                        }
                    }
                }
            ...
            case ("repeat all"):
                ...
                if (playMode.equals("album")) {
                    while (playTime > currentAlbum.getSongs().get(songIndexAlbum).getDuration()) {
                        ...
                        currentAlbum.getSongs().get(songIndexAlbum).increaseListeners();
                    }
                } else {
                    while (playTime > currentPlaylist.getSongs().get(songIndex).getDuration()) {
                        playTime -= currentPlaylist.getSongs().get(songIndex).getDuration();
...
//        newNode.put("shuffle", shuffle);
    ...
    public boolean like(final Command command) {
        ...
        } else if (playMode.equals("album")) {
            song = currentAlbum.getSongs().get(songIndexAlbum);
        }
    ...
    public void prev(final Command command, final ObjectNode resultNode) {
        ...
        switch (playMode) {
            ...
            case "podcast":
                ...
                break;
            case "album":
                if (songIndexAlbum != 0 && playTime == 0 && !paused && userStatus.equals("online")) {
                    songIndexAlbum--;
                } else {
    ...
    private void unsuffle() {
        if (playMode.equals("album")) {
            songIndexAlbum = unsuffledSongsAlbum.indexOf(currentAlbum.getSongs().get(songIndexAlbum));
